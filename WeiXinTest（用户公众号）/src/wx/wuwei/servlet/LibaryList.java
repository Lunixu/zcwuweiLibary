...
/**
 * Servlet implementation class LibaryList
 */
@WebServlet("/LibaryList")
public class LibaryList extends HttpServlet {
            ...
            if(weixinOauth2Token!=null)
            {
                ...
                try 
                {
					if(ShowUserinfoService.judgeUserInfo(openId))//若是绑定用户
					{
					    ...
					    System.out.println("66666"+session.getAttribute("readerid"));
					}
				} 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			 ...
			 request.setAttribute("libaryinfo", pal);
			 request.getRequestDispatcher("LibaryList.jsp").forward(request,response);
		}
		catch(Exception ex)
		{
			ex.printStackTrace();
